-- MJ Brand OS — FINAL production schema (run once in SQL Editor)
-- Order: run this entire file after project is created

-- 1) Storage bucket (public read for image URLs)
...
-- 2) App state (per user, JSON document)
...
-- 3) Realtime (Dashboard → Database → Replication: enable mj_app_state)
